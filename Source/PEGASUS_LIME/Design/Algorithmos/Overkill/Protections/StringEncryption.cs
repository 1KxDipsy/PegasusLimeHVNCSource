...
using PEGASUS_LIME.Design.Algorithmos.Overkill.Utils;
...
namespace PEGASUS_LIME.Design.Algorithmos.Overkill.Protections
{
	public class StringEncryption : Randomizer
	{
		...
		{
			ModuleDefMD moduleDefMD = ModuleDefMD.Load(typeof(StringDecoder).Module);
			TypeDef typeDef = moduleDefMD.ResolveTypeDef(MDToken.ToRID(typeof(StringDecoder).MetadataToken));
			IEnumerable<IDnlibDef> source = InjectHelper.Inject(typeDef, Ovelkill.Module.GlobalType, Ovelkill.Module);
			MethodDef methodDef = (MethodDef)source.Single((IDnlibDef method) => method.Name == "Decrypt");
